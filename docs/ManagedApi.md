...
> BlueprintImportResult managed_blueprints_import_create(file=file, path=path, context=context)
...
with authentik_client.ApiClient(configuration) as api_client:
    ...
    try:
        api_response = api_instance.managed_blueprints_import_create(file=file, path=path, context=context)
        print("The response of ManagedApi->managed_blueprints_import_create:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling ManagedApi->managed_blueprints_import_create: %s\n" % e)
```
