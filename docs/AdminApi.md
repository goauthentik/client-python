...
with authentik_client.ApiClient(configuration) as api_client:
    # Create an instance of the API class
    api_instance = authentik_client.AdminApi(api_client)
    settings_request = authentik_client.SettingsRequest() # SettingsRequest |  (optional)
...
# **admin_workers_list**
> List[Worker] admin_workers_list()
...
with authentik_client.ApiClient(configuration) as api_client:
    ...
    try:
        api_response = api_instance.admin_workers_list()
        print("The response of AdminApi->admin_workers_list:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling AdminApi->admin_workers_list: %s\n" % e)
```
