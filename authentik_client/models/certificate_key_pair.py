...
class CertificateKeyPair(BaseModel):
    ...
    pk: UUID
    name: StrictStr
    fingerprint_sha256: Optional[StrictStr] = Field(description="SHA256 fingerprint of the certificate")
    fingerprint_sha1: Optional[StrictStr] = Field(description="SHA1 fingerprint of the certificate")
    cert_expiry: Optional[datetime] = Field(description="Certificate expiry date")
    cert_subject: Optional[StrictStr] = Field(description="Certificate subject as RFC4514 string")
    private_key_available: StrictBool = Field(description="Show if this keypair has a private key configured or not")
    key_type: Optional[CertificateKeyPairKeyTypeEnum] = Field(description="Key algorithm type detected from the certificate's public key")
